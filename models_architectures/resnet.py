...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, use_bn=True):
        ...
        self.conv1 = nn.Conv2d(
            in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=not use_bn)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, use_bn=True):
        ...
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=1, bias=not use_bn)
        self.bn1 = nn.BatchNorm2d(planes) if use_bn else nn.Identity()
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                               stride=stride, padding=1, bias=not use_bn)
        self.bn2 = nn.BatchNorm2d(planes) if use_bn else nn.Identity()
        self.conv3 = nn.Conv2d(planes, self.expansion *
                               planes, kernel_size=1, bias=not use_bn)
        self.bn3 = nn.BatchNorm2d(self.expansion*planes) if use_bn else nn.Identity()
...
def ResNet18(num_classes, use_bn=True):
    return ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, use_bn=use_bn)
