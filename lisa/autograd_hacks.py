    ...
    for layer in model.modules():
        ...
        if layer_type == 'Linear':
            ...
            if layer.padding_mode == "zeros":
                ...
                padding = 0
                A = torch.nn.functional.pad(A, (int(layer.padding[0]), int(layer.padding[0]), int(layer.padding[1]), int(layer.padding[1])),
                                            mode=layer.padding_mode)
            groups = layer.groups
            A = torch.nn.functional.unfold(A, layer.kernel_size, padding=padding, stride=layer.stride)
            A = A.view(n*groups, -1, A.shape[-1])
            B = B.reshape(n*groups, -1, A.shape[-1])
            ...
            if layer.affine:
                setattr(layer.weight, 'grad1', torch.sum(B * A, dim=(-2,-1)))
                setattr(layer.bias, 'grad1', torch.sum(B, dim=(-2,-1)))
