...
class AdaBelief(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, acc=0., betas=(0.9, 0.999), eps=1e-16,
                 weight_decay=0, amsgrad=False, weight_decouple=False, fixed_decay=False, rectify=False,
                 degenerated_to_sgd=False, print_change_log = False, writer=None):
        ...
        defaults = dict(lr=lr, acc=acc, betas=betas, eps=eps,
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    state['step'] = 0
                    # Exponential moving average of gradient values
                    state['exp_grad'] = torch.zeros_like(p.data,memory_format=torch.preserve_format) \
                        if version_higher else torch.zeros_like(p.data)
                    state['exp_avg'] = torch.zeros_like(p.data,memory_format=torch.preserve_format) \
                        if version_higher else torch.zeros_like(p.data)
                    # Exponential moving average of squared gradient values
                ...
                exp_grad, exp_avg, exp_avg_var = state['exp_grad'], state['exp_avg'], state['exp_avg_var']

                state['step'] += 1
                bias_correction0 = 1 - acc ** state['step']
                # bias_correction1 = 1 - beta1 ** state['step']
                bias_correction2 = 1 - beta2 ** state['step']

                # Update first and second moment running average
                exp_grad.mul_(acc).add_(grad, alpha=1 - acc)
                ...
                if not self.rectify:
                    ...
                    p.data.addcdiv_( exp_grad, denom, value=-step_size)  # FIXME: exp_avg
                ...
                if half_precision:
                    p.data = p.data.half()
                    p.grad = p.grad.half() 
                denoms.append(denom.ravel())

        self.k += 1
        A_k = torch.concat(denoms)

        self._log("alpha", group['lr'])
        self._log("min(A_k)", A_k.min())
        self._log("max(A_k)", A_k.max())
        self._log("min(D_k)", 1/A_k.max())
        self._log("max(D_k)", 1/A_k.min())
